...
def Sequence_fast_forward(fast_forward_inference):
    def _Sequence_fast_forward(
        ...
        *args, **kwargs,
        ...
        bsz, q_len, hd = hidden_states.shape # bsz=16, q_len=2300, hd=3584
        
        score = self.score.weight
        # lm_head = self.lm_head.weight # self.lm_head: in_features = 3584 out_features = 256000
        if bsz == 1 and q_len == 1:
            ...
            logits = self.score(hidden_states[:, -num_logits_to_keep:, :].to(score.dtype))
        else:
            # logits = self.lm_head(hidden_states.to(lm_head.dtype)) # torch.Size([16, 2300, 256000])
        ...
        if labels is not None:
            ...
            # print(labels.shape)
            # print(self.extra_ignored_labels.shape)
            ...
            raise ValueError("Unsloth: You must provide labels for training!")
        pass
...
class FastLlamaModelSequenceClassification:
    ...
    def pre_patch():
        ...
        PeftModelForSequenceClassification.forward.forward = PeftModelForSequenceClassification_fast_forward
